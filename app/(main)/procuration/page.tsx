...
//                     <SelectItem value="administrative">Procuration administrative</SelectItem>
//                     <SelectItem value="judiciaire">Procuration judiciaire</SelectItem>
//                   </SelectContent>
//                 </Select>
//               </div>
...
//                 </div>
//                 <div>
//                   <Label htmlFor="justificatifDomicile">justificatif de Domicile du Mandant</Label>
//                   <div className="mt-1 flex items-center">
//                     <label htmlFor="justificatifDomicile" className="cursor-pointer bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
//                       <Upload className="h-5 w-5 inline-block mr-2" />
//                       Choisir un fichier
//                     </label>
//                     <input
...
//               <Button type="submit" className="w-full">Télécharger et envoyer</Button>
//             </form>
//           </CardContent>
//         </Card>
...
export default function ProcurationComplete() {
  ...
  const handleDateInputChange = (name: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target
    const date = parse(value, 'dd/MM/yyyy', new Date())
    if (isValid(date)) {
      setFormData(prev => ({ ...prev, [name]: date }))
    } else {
      // Si la date n'est pas valide, on garde la valeur saisie dans l'input
      // mais on ne met pas à jour la date dans le state
      setFormData(prev => ({ ...prev, [name]: value }))
    }
  }
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className="w-1/2">
              <CalendarIcon className="mr-2 h-4 w-4" />
              {formData[name as keyof FormData] instanceof Date ? 
                format(formData[name as keyof FormData] as Date, 'P', { locale: fr }) : 
          ...
          <PopoverContent className="w-auto p-0">
            <Calendar
              mode="single"
              selected={formData[name as keyof FormData] instanceof Date ? formData[name as keyof FormData] as Date : undefined}
              onSelect={(date) => {
                if (date) {
                  setFormData(prev => ({ ...prev, [name]: date }))
                }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {renderDateInput('dateNaissanceMandataire', 'Date de naissance')}
                <div>
                  <Label htmlFor="lieuNaissanceMandataire">Lieu de naissance</Label>
          ...
          <CardContent>
            ...
            >
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Je soussigné(e),</h3>
                <p><strong>Mandant :</strong> {formData.prenomMandant} {formData.nomMandant}</p>
                <p><strong>Né(e) le :</strong> {formData.dateNaissanceMandant instanceof Date ? format(formData.dateNaissanceMandant, 'P', { locale: fr }) : formData.dateNaissanceMandant || "Non spécifié"}</p>
                <p><strong>À :</strong> {formData.lieuNaissanceMandant}</p>
                <p><strong>Nationalité :</strong> {formData.nationaliteMandant}</p>
                <p><strong>Demeurant à :</strong> {formData.adresseMandant}</p>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Donne par la présente pouvoir à :</h3>
                <p><strong>Mandataire :</strong> {formData.prenomMandataire} {formData.nomMandataire}</p>
                <p><strong>Né(e) le :</strong> {formData.dateNaissanceMandataire instanceof Date ? format(formData.dateNaissanceMandataire, 'P', { locale: fr }) : formData.dateNaissanceMandataire || "Non spécifié"}</p>
                <p><strong>À :</strong> {formData.lieuNaissanceMandataire}</p>
                <p><strong>Nationalité :</strong> {formData.nationaliteMandataire}</p>
                <p><strong>Demeurant à :</strong> {formData.adresseMandataire}</p>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg">Durée de la procuration :</h3>
                <p><strong>Du :</strong> {formData.dateDebut instanceof Date ? format(formData.dateDebut, 'P', { locale: fr }) : formData.dateDebut || "Non spécifié"}</p>
                <p><strong>Au :</strong> {formData.dateFin instanceof Date ? format(formData.dateFin, 'P', { locale: fr }) : formData.dateFin || "Non spécifié (durée indéterminée)"}</p>
              </div>

              <div className="pt-6 border-t">
                <p><strong>Fait à :</strong> {formData.lieuSignature}</p>
                <p><strong>Le :</strong> {formData.dateSignature instanceof Date ? format(formData.dateSignature, 'P', { locale: fr }) : formData.dateSignature || "Non spécifié"}</p>
              </div>
