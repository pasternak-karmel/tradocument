...
import { Checkbox } from "@/components/ui/checkbox";
import { Mail, User, FileText, Briefcase, Scale, GraduationCap, CreditCard, Truck, FileSignature } from 'lucide-react';
import { calculateMontantPage } from "@/actions/calculate_montant_page";
import { showError } from "@/function/notification-toast";
import { acceptedFileTypes, languages } from "@/type";
import { toast } from "sonner";
                ...
                >
                  ...
                  <FormField
                    ...
                    render={() => (
                      <FormItem>
                        <div className="mb-4">
                          <FormLabel className="text-base">Types de documents</FormLabel>
                        </div>
                        {translationCategories.map((category) => (
                          <div key={category.title} className={`mb-4 p-4 rounded-lg ${category.color}`}>
                            <button
                              type="button"
                              onClick={() => toggleCategory(category.title)}
                              className="w-full text-left text-lg font-semibold mb-2 flex items-center justify-between"
                            >
