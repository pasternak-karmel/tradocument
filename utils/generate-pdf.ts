...
export async function generatePDF(data: ProcurationFormData) {
  ...
  drawText(
    `Je, soussigné(e) ${data.prenom} ${data.nom.toUpperCase()}`,
    140,
    y,
    timesBold
  );
  ...
  drawText(
    `titulaire de la pièce d'identité numéro ${data.numeroIdentite},`,
    50,
    y
  );
  y -= lineHeight;
  ...
  if (data.dateLimite) {
    drawText(
      `Cette procuration est valable jusqu'au ${format(
        new Date(data.dateLimite),
        "dd/MM/yyyy",
        { locale: fr }
      )}.`,
      50,
    ...
    drawText(
      `Fait à ${data.lieuSignature}, le ${format(
        new Date(data.dateSignature),
        "dd/MM/yyyy",
        { locale: fr }
